...
export function useAuthProvider() {
  ...
  const setSignData = async (data: any) => {
    const signData = data.data.data;
    try {
      setIsFirstLogin(signData.isFirstLogin);
      setStorageAccessToken(signData.accessToken);
      setToken(signData.accessToken);
      setStorageRefreshToken(signData.refreshToken);
      return { err: false, data: data };
    } catch (e: any) {
      console.log('signIn error', e);
      return {
