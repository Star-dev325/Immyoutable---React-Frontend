...
export function useMetamaskProvider() {
  ...
  // Get Nonce For Auto Sign In From Backend Start
  const getNonce = async () => {
    try {
      const { data } = await axiosApiInstance.post(
        'public/crypto-auth/metamask/nonce',
        {
          address: address
        }
      );
      console.log('getCryptoAccount data', data);
      if (data?.success) {
        return {err: false, data: data.data.nonce}
      }

      return {err: true, data: data}
    } catch (e) {
      return {err: true, data: 'Server err'}
    }
  };
  // Get Nonce For Auto Sign In From Backend End

  // Sign In With Nonce Start
  const signInWithNonce = async () => {
    try {
      ...
      const { data } = await axiosApiInstance.post(
        'public/crypto-auth/metamask/verify',
        {
          address,
          message,
          signature
